...
static_assert(sizeof(Vertex) == 32, "scalar layout: three vec3 is 32 bytes");
...
protected:
    ...
    std::array<VkDescriptorSetAndBindingMappingEXT,6> mappings(){
        ...
        m[5].sourceData.constantOffset.heapOffset = faceTextureSlot_.offset;
    ...
    void onInit() override {
        ...
        texture_ = VulkanTextures::loadRgba8(
            context(), "assets/container.jpg", 
            TEXTURE_COLOR_SPACE::SRGB, MIP_MODE::GENERATE);
        faceTexture_ = VulkanTextures::loadRgba8(context(), "assets/awesomeface.png", 
            TEXTURE_COLOR_SPACE::SRGB, MIP_MODE::GENERATE);


        assert(faceTexture_.extent().width == 512);
        assert(texture_.extent().height == 512);
        assert(faceTexture_.mipLevels() == 10);
        assert(texture_.layout() == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        ...
        faceTextureSlot_ = heap_.allocate(context(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE); 
        ...
        heap_.writeImage( faceTextureSlot_, faceTexture_.viewInfo(), faceTexture_.layout());

        SDL_Log("face tex offset: %u", faceTextureSlot_.offset );
        sampler.maxLod = static_cast<float>(texture_.mipLevels() - 1);
        samplerHeap_.write(samplerSlot_, sampler);
        SDL_Log("[heap] container slot %u, face slot %u", textureSlot_.offset, faceTextureSlot_.offset);
...
private:
    static ExampleConfig makeConfig() {
        ExampleConfig cfg;
        cfg.name   = "05_texture_mapping";
        cfg.title  = "05 - texture mapping";
        cfg.width  = 1280;
        cfg.height = 720;
        cfg.depth  = true;
    ...
    VulkanTexture  faceTexture_;
    DescriptorSlot faceTextureSlot_;
